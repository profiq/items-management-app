...
We use Auggie for automated code review. This code review runs when an MR is non-draft and is labelled as "Waiting for review". It uses the following variables specified in the project CI/CD settings:

```
GLAB_TOKEN
AUGMENT_CLI_SESSION_KEY
AUGMENT_RETRY_TIMEOUT
```

Where `GLAB_TOKEN` is the access token for the GitLab service account with expiration period of one year. It needs to be replaced when it expires. It needs the following permissions: `api`, `read_repository`, `user`.

`AUGMENT_CLI_SESSION_KEY` is the Augment Code non-interactive token.

`AUGMENT_RETRY_TIMEOUT` is an optional variable that allows specifying the rate-limit retry timeout. It defaults to 120 seconds.

We use three skills for code review:

- [Code Review Skill](.augment/skills/code-review/SKILL.md) - Analysis of a GitLab MR or local changes.
- [Code Review Filter Skill](.augment/skills/code-review-filter/SKILL.md) - Filter out the duplicates out of the Code Review Skill using existing GitLab discussions.
- [Code Review Poster Skill](.augment/skills/code-review-poster/SKILL.md) - Post code review to a given GitLab MR in new discussions and diff notes.

These skills are also exposed using three commands:

- [`/review-code`](.augment/commands/review-code.md) - Reviews either local code changes or GitLab MR changes (using "mr:<MR Number>" argument) and saves the output to a JSON file.
- [`/filter-review`](.augment/commands/filter-review.md) - Loads the saved JSON file generated by `/review-code` and uses the filter skill to filter out duplicates and saved the non-duplicates to a JSON file.
- [`/post-review`](.augment/commands/post-review.md) - Loads the saved JSON file generated by `/filter-review` and uses the poster skill to post the reviews to a given MR.

All GitLab communication happens through GitLab CLI MCP server (`glab mcp serve`).

For more detailed information about usage, see [.augment/commands/README.md](.augment/commands/README.md).

### Token Renewal

The `GLAB_TOKEN` expires annually and must be renewed:

1. Generate a new token:
   - Go to GitLab -> Settings -> Service Accounts -> Manage access tokens
   - Create a token with scopes: `api`, `read_repository`, `user`
   - Set expiration to 1 year from now

2. Update the CI/CD variable:
   - Go to Project -> Settings -> CI/CD -> Variables
   - Edit `GLAB_TOKEN` and paste the new token
   - Mark as "Masked" and "Hidden"

3. Test the token:

```bash
glab auth login -t <new-token>
glab auth status
```
...
flowchart LR
    ...
    subgraph sub_code_review[code_review]
        direction LR
        node_code_review[code_review]
    end
    sub_test --> sub_code_review
    style node_code_review fill:cyan
